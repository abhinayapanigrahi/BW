...
class userLoginProcessor extends applicationProcessor {
	...
	public function checkLogin($params,$dbconn)
	{
		...
		echo  $sql_Login =  "SELECT *,DATE_FORMAT(last_loggedin,'%D %b %Y, %W') lastLoggedIn FROM ".TABLE_USER_MASTER." WHERE user_name = '".$username."' AND user_password = '".md5($password)."'";
		...
		if(mysql_num_rows($res) == 1)
		{
			...
			if($objUser->desig_id == 1){
				header("location: ../../");
			}else if($objUser->desig_id == 2){
				header("location: ../../vendors/".trim($objUser->venderweb));					
			}
